...
*About 317_04s
In this version, we will plot resutls for different NH4 levels while keeping carbon constant
#----------------------------------------------------------------------------------------------
#note 
...
##########################################################################################################################################
def kkk(O2satratio,CHinsource):
    #@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@
    #1.Parameters----------------------------
    #@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@
    O2sat=0.225         #(mol m-3) Saturated oxygen concentration (Post et al 1982)
    #O2satratio=0.3         #(diemnsionless) The saturation ratio of oxygen
    global O2percent
...
#    plotcolor=kuhla_data.plotcolor
    ...
    Ninmax=10
    Ninnstep=100
    Ninstep=(Ninmax-Ninmin)/Ninnstep
    global Nin
    Nin=arange(Ninmin+Ninstep,Ninmax+Ninstep,Ninstep)   #(molN m-3): N concentration in the poring media
    Ninnumber=size(Nin)
    U=arange(0,Ninnumber,1)
...
#    CHin=100    #(mol/m3): CH concentration in the poring water
   ...
   # df=0.001
    ...
    far0=arange(df,1+df,df)          #f arange
    
    far=far0.reshape(size(far0),1)
    
    CH=copy(far)                    #This is the only one that I use
    EEi=copy(far)/copy(far)
    xnh4=copy(EEi)*copy(U)
    a7=CNwidth/CNstep
    res2grow=copy(EEi)
    res2=copy(EEi)
    Ncb=copy(EEi)
    ...
    for j in U:
        xch1[j]=(D*(CHin-CH0))/((lsgrow*(1+E3+F)+m)*Rv)  
        N1[j]=(D*Nin[j]-xch1[j]*Rv*lsgrow*r4)/D         #(mol m-3) ammonium concentration in the vessel assuming (158-33)
        ...
        if O2cpot[j]>O2cri:             #When there is not enough carbon to reduc oxygten concentration when only all ammonium is used
            ...
            if xch1[j]>xn[j]:              #this part is to make sure that the right value is chosen when O2c<O2critical when no extra respiration
                x[j]=xn[j]                 # this case is when extra respiration is done
                res[j]=res1[j] 
                CHp[j]=CHpot[j]
                NH4[j]=N 
                whichone[j]=3
            ...
            xch0=D*(CHin-CH[0])/(4*pi*R*Dch*(CH[0]-CHc))/Rv     #(cell/m3): number density of the cells
            if xnh4[0,j]<xch0:
                while (imax-imin>1):
                    center=(imax+imin)//2
                    xch=D*(CHin-CH[center])/(4*pi*R*Dch*(CH[center]-CHc))/Rv     #(cell/m3): number density of the cells
                    xnc=xch-xnh4[center,j]
    ...
    #Calculation of N2 fixation per volume
    #OOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOO
    N2fix_v=N2fix*PRd           #(nmol NH4+ min-1 mL-1) Nitorgen fixation per volume
    #print(N2fix)
    #--------------------------------------------------
...
#===========================
#O2 concentration variation
#===========================
O2satratiostep=0.01
O2satratiomax=1
O2satratioarray=arange(O2satratiostep,O2satratiomax+O2satratiostep,O2satratiostep)  #(mol O2 m-3) Oxygen concentration in the environment
O2percentarray=O2satratioarray*100

CHinsourcearray=array([400])
for c in CHinsourcearray:
    for b in O2satratioarray:
        N2fix,PR,Resp,Yp=kkk(b,c)
        if b==O2satratiostep:
            N2fixbox=N2fix
            PRbox=PR
            Respbox=Resp
    ...
    #oooooooooooooooooooooooooo
    axes="N-O2"
    First_part="C:\\Users\\Keisuke\\Desktop\\figures\\Azoto2\\2Dplot\\"+axes+"\\"
    #Second part==========
    Plot1="N2fix\\"
    Plot2="Protein\\"
    Plot3="Respiration\\"
    Plot5="N2fix_volume\\"
    #=====================
    Last_part=CHin+" mol C sucrose.png"
    #oooooooooooooooooooooooooo
    
    figure(1)
    pcolormesh(Nin,O2percentarray,N2fixbox,vmax=32)
    xlabel('NH$_4^+$ (mol N m$^{-3}$)')
    ylabel('O$_2$ ($\%$)')
    title(CHin+' mol C sucrose m$^{-3}$')
    colorbar().set_label('N$_2$ fixation (nmol min$^{-1}$ mg protein$^{-1}$)')
    savetxt("2DN2fix.csv", N2fixbox, delimiter=",",fmt='%2.8f')
   # savefig(First_part+Plot1+Last_part)  
