...
#    CHin=100    #(mol/m3): CH concentration in the poring water
    ...
    #=====================================================================================    
    df=0.001                            #plese make small enoough value and value that can divide 1 evenly  (e.g. 0.001 etc.) 
    floop=arange(0,1/df-1,1)          #f for loop
    far=arange(df,1,df)          #f arange
    ...
    for j in U:
        ...
        if O2cpot[j]>O2cri:             #When there is not enough carbon to reduc oxygten concentration when only all ammonium is used
            if xch1[j]>xn:              
                ...
                if ExResp==1:
                    res[j]=res1[j]-CCHextraresp_a[j]
                    CCHextraresp[j]=0#CCHextraresp_a[j]
            else:
                x[j]=xch1[j]
                res[j]=res0
                CHp[j]=CH0
            ...
            for i in floop:
                ...
                if xnh4[i]>xch:
                    ...
                    whichone[j]=5
                    Epp[j]=Ep[i]
                    CCHbiomass[j]=CCHbiomass_b[i]
                    CCHN2fix[j]=CCHN2fix_b[i]
                    CCHrespbiomass[j]=CCHrespbiomass_b[i]
                    CCHrespN2fix[j]=CCHrespN2fix_b[i]
                    CCHextraresp[j]=CCHextraresp_b[i]                        
                    break
                else:
                    x[j]=xch
                    res[j]=res2[i]
...
#     xlim(xmax=Xlimmax)
#     if buhler_data.i==buhler_data.imax:
#         Savefig(Folderloc,5)
    ...
    figure(13+buhler_data.i)
    if ExResp==1:
        plot(CN, resp, '--',linewidth=5, color='black')#, label='O$_2$=' +str(int(O2/0.225*100))+'$\%$')
    else:
        plot(CN, resp, linewidth=2.5, color=plotcolor)#, label='O$_2$=' +str(int(O2/0.225*100))+'$\%$')
    plot(buhler_data.cnre, buhler_data.red,'o', color=plotcolor)
    xlabel(xname)
    ylabel('Respiration (umol O$_2$ min$^{-1}$ mg protein$^{-1}$)',fontsize=23) 
    xlim(xmax=Xlimmax)  
   # if buhler_data.i==buhler_data.imax:
  #  Savefig(Folderloc,13+buhler_data.i)
...
#     figure(14)
#     plot(CN, CHs, linewidth=2.5, color=plotcolor)
#     plot(buhler_data.cnch, buhler_data.chd,'o', color=plotcolor)
#     xlabel(xname)
#     ylabel('CH mM'' ')
#     xlim(xmax=Xlimmax)
#     if buhler_data.i==buhler_data.imax:
#         Savefig(Folderloc,14)
    #OOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOO
    ######################################
    
#     figure(30)
#     plot(CN, whichone, linewidth=2.5, color=plotcolor)
#     xlabel(xname)
#     xlim(xmax=Xlimmax)
#     if buhler_data.i==buhler_data.imax:
#         Savefig(Folderloc,30)
    #########################
    #OOOOOOOOOOOOOOOOOOOOOOOO

#     figure(15)
#     if buhler_data.i!=2: 
#         plot(CN, Yp, linewidth=2.5, color=plotcolor)
#     plot(buhler_data.cnY, buhler_data.Yd,'o', color=plotcolor)
#     xlabel(xname)
#     ylabel('Yield (g biomss / mol C sucrose) ')
#     xlim(xmax=Xlimmax)
#     if buhler_data.i==buhler_data.imax:
#         Savefig(Folderloc,15)
    ...
    #OOOOOOOOOOOOOOOOOO    
#     figure(17)
#     plot(CN, fp, linewidth=2.5, color=plotcolor)
#     xlabel(xname)
#     ylabel('f')
#     xlim(xmax=Xlimmax)
#     if buhler_data.i==buhler_data.imax:
#         Savefig(Folderloc,17)
    #OOOOOOOOOOOOOOOOOO
    ###################    
    
    ###################    
    #OOOOOOOOOOOOOOOOOO
    rcParams.update({'lines.markeredgewidth': 3})    
    
#     fig=figure(18)
#     ax1=fig.add_subplot(111)
#     ax2=ax1.twinx()
#     ax1.plot(CN, N2fix, linewidth=2.5, color=plotcolor)
...
#     ax1.set_ylabel('N$_2$ fixation rate (nmol min$^{-1}$ mg protein$^{-1}$)',fontsize=22)
#     ax1.set_ylim(0,35)
#     ax2.set_ylim(0,35/fp_to_N2fix*100)
#     ax2.set_ylabel('N$_2$ fixation ratio ($\%$)')
#     xlim(xmax=Xlimmax)
#     if buhler_data.i==buhler_data.imax:
#         Savefig(Folderloc,18)
...
#     Savefig(Folderloc,Figurenumber+buhler_data.i)
    ...
    #########################
    #OOOOOOOOOOOOOOOOOOOOOOOO
    StackPlotColors2=('#008000','red','#0000FF','#FFFF00','cyan')
    
    figure(Figurenumber+1+buhler_data.imax+buhler_data.i)
    stackplot(CN,CCHbiomass_plot,CCHN2fix_plot,CCHrespbiomass_plot,CCHrespN2fix_plot,CCHextraresp_plot\
                  , colors=StackPlotColors2)
    xlabel(xname)
    ylabel('$C_S$ (mmol h$^{-1}$ g protein$^{-1}$)')  #copied from 73
    xlim(xmax=Xlimmax)
    ylim(ymax=16)
    ax=gca()                                                        #the following two lines are for limiting the digits 
    ax.yaxis.set_major_formatter(FormatStrFormatter('%.f'))
  #  Savefig(Folderloc,Figurenumber+1+buhler_data.imax+buhler_data.i)
    #legend(loc='upper right', borderaxespad=0.6, fontsize=23)
    
    print(buhler_data.i)
    
#     figure(55)
#     plot(CN, CCHbiomass_plot)
    return 
    ##########################################################################################################################################
#@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@
#6.execution
