...
#                 else:
   ...
   # CCHbalancedresp_plot=CCHbalancedresp*conversion1     #(mmolSucrose/(h*g protein)) CCH for balanced respiration
    ...
    for j in U:
        if j>0:
            if mode[j]==2 and mode[j-1]==1:
                B12=CN[j]*30
            if mode[j]==3 and mode[j-1]==2:
                B23=CN[j]*30
    
    return N2fix,PRd,resp,Yp,mode,B12,B23
##########################################################################################################################################
#@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@
#6.execution
#@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@
...
B12array=zeros(size(Ninarray))
B23array=zeros(size(Ninarray))

count=0
for a in Ninarray:
    #start = time.time()   #current time in seconds
    N2fix,PR,Resp,Yp,mode,B12,B23=kkk(a)
    ...
    if a==Ninstep:
        ...
        modebox=vstack((modebox,mode))

    B12array[count] = B12
    B23array[count] = B23
    if count>0:
        if (B12array[count-1]>0 and B12array[count]==0) or isnan(B12array[count-1])==True:
            B12array[count]=nan
        if (B23array[count-1]>0 and B23array[count]==0) or isnan(B23array[count-1])==True:
            B23array[count]=nan
...
# Plot5="N2fix_v\\"
#=====================
Last_part=O2p+"percent O2.png"
#oooooooooooooooooooooooooo

figure(1)
pcolormesh(CHin,Ninarray,N2fixbox,vmax=32,vmin=0)
xlabel('Sucrose (mol C m$^{-3}$)')
ylabel('NH$_4^+$ (mol N m$^{-3}$)')
title(O2p+'$\%$ O$_2$')
colorbar().set_label('N$_2$ fixation (nmol min$^{-1}$ mg protein$^{-1}$)')
savetxt("2DN2fix"+O2p+".csv", N2fixbox, delimiter=",",fmt='%2.8f')
...
# savetxt("2DPR"+O2p+".csv", PRbox, delimiter=",",fmt='%2.8f')
# savefig(First_part+Plot2+Last_part) 

# figure(3)
# pcolormesh(CHin,Ninarray,Respbox,vmax=2.7,vmin=0)
# xlabel('Sucrose (mol C m$^{-3}$)')
# ylabel('NH$_4^+$ (mol N m$^{-3}$)')
# title(O2p+'$\%$ O$_2$')
# colorbar().set_label('Respiration (umol O$_2$ min$^{-1}$ mg protein$^{-1}$)')
# savetxt("2DResp"+O2p+".csv", Respbox, delimiter=",",fmt='%2.8f')
# savefig(First_part+Plot3+Last_part)

# figure(4)
# pcolormesh(CHin,Ninarray,Ypbox)
# xlabel('Sucrose (mol C m$^{-3}$)')
# ylabel('NH$_4^+$ (mol N m$^{-3}$)')
# title(O2p+'$\%$ O$_2$')
# colorbar().set_label('Yield (g biomss mol C sucrose$^{-1}$)')
# savetxt("2DYp"+O2p+".csv", Ypbox, delimiter=",",fmt='%2.8f')

figure(5)
pcolormesh(CHin,Ninarray,N2fixbox*PRbox,vmax=50,vmin=0)
xlabel('Sucrose (mol C m$^{-3}$)')
ylabel('NH$_4^+$ (mol N m$^{-3}$)')
title(O2p+'$\%$ O$_2$')
colorbar().set_label('N$_2$ fixation (nmol N min$^{-1}$ ml$^{-1}$)')
#savefig(First_part+Plot5+Last_part)
savetxt("2DN2fixV"+O2p+".csv", N2fixbox*PRbox, delimiter=",",fmt='%2.8f')

# figure(6)
#pcolormesh(CHin,Ninarray,modebox)
# plot(B12array,Ninarray)
#colorbar()

# figure(7)
# pcolormesh(CHin,Ninarray,modebox)
# plot(B12array,Ninarray,color='w')
# plot(B23array,Ninarray,color='g')

savetxt("B12array"+O2p+".csv", B12array, delimiter=",",fmt='%2.8f')
savetxt("B23array"+O2p+".csv", B23array, delimiter=",",fmt='%2.8f')

savetxt("Ninarray"+O2p+".csv", Ninarray, delimiter=",",fmt='%2.8f')
savetxt("CHin"+O2p+".csv", CHin, delimiter=",",fmt='%2.8f')
